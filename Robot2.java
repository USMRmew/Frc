...
public class Robot extends IterativeRobot {
	...
	public void autonomousInit() {
		...
		// Custom SmartDashboard values for autonomousPeriodic method
		SmartDashboard.putNumber("Stage 1 Distance", stage1);
		SmartDashboard.putNumber("Turn 1 Rotation", turn1);
		SmartDashboard.putNumber("Stage 2 Distance", stage2);
		SmartDashboard.putNumber("Turn 2 Rotation", turn2);
		SmartDashboard.putNumber("Stage 3 Distance", stage3);
		SmartDashboard.putNumber("Turn 3 Rotation", turn3);
		SmartDashboard.putNumber("Stage 4 Distance", stage4);
		SmartDashboard.putNumber("Turn 4 Rotation", turn4); // Continue for more stages in autonomous (including more distances and possibly switch methods)
		SmartDashboard.putNumber("turn right calibrate", turnR);
		SmartDashboard.putNumber("turn left calibrate", turnL);
	...
	public void turnLeft(double input) {
		input /= 100;
		double rotate = 17.3 * input;
		encoderRight.reset();
		while (encoderRight.getDistance() < rotate) {
			Robot.tankDrive(-.3, .3, false);
			//17.5
		}
		System.out.println("TurnLeft has executed");
		Robot.tankDrive(0,0);
	}
	...
	public void turnRight(double input) {
		input = input / 100;
		double rotate = 13.5 * input;
		//20
		encoderRight.reset();
		while (-encoderRight.getDistance() < rotate) {
			Robot.tankDrive(.3, -.3, false);
		}
		System.out.println("TurnRight has executed");
		Robot.tankDrive(0,0);
	}
	 ...
	 * motor correction (DEPRI(E*)CATED)
	 */
	...
	public void autonomousPeriodic() {
		...
		*/
		...
		// SMARTDASHBOARD VALUES FOR AUTONOMOUSPERIODIC
		SmartDashboard.putNumber("Stage 1 Distance", stage1);
		SmartDashboard.putNumber("Turn 1 Rotation", turn1);
		SmartDashboard.putNumber("Stage 2 Distance", stage2);
		SmartDashboard.putNumber("Turn 2 Rotation", turn2);
		...
		SmartDashboard.putNumber("Turn 3 Rotation", turn3);
		SmartDashboard.putNumber("Stage 4 Distance", stage4);
		SmartDashboard.putNumber("Turn 4 Rotation", turn4);
		
		SmartDashboard.putNumber("Switch Configuration Speed", switch1);
		
		switch(autoCase) {
		
		// INSERT SMARTDASHBOARD GET VALUES HERE FOR AUTONOMOUS
		...
		if (count == 400) {
			...
			SmartDashboard.putNumber("Left Average", sumL / 400.0);
		  		  ...
		  		  if (m_operatorControl.getRawAxis(1) > 0 || m_operatorControl.getRawAxis(1) < 0) {
		  		  	forkLift.tankDrive(m_operatorControl.getRawAxis(1)*-1, m_operatorControl.getRawAxis(1)*-1, false);
		  		  }
